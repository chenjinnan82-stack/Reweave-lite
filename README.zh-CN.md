...
Static Web 目标 Patch 契约目前只属于后端应用服务；公开 CLI 和桌面目标接入页面尚未提供该入口。
...
- 经 `ReweaveAppService` 分析一个显式选择的 Static Web 目标入口，并返回确定性、绑定目标快照的 Weave Plan 和完整 review-only Patch。
- 默认关闭真实源项目写入。
...
Static Web 目标后端只返回 Patch 数据；目前没有目标接入 UI，不应用 Patch、不 commit，也不宣称自动完成法律许可证授权判断。
...
唯一权威产品路线图是 [Reweave 产品北极星](docs/REWEAVE_PRODUCT_NORTH_STAR.md)。其中保留四个可独立验收的计划：Static Web 后端切片已经完成，独立的前端交互闭环仍待实施。
