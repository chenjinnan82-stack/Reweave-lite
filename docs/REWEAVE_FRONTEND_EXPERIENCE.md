...
- 当前事实基线：2026-07-20，胶囊仓库场景基线 commit `be0da04`
...
- 真实 SQLite 正式仓库上的只读蛛网场景，包含多项目／多胶囊布局、精确版本正式来源关系、搜索、缩放、返回和键盘路径。
...
因此，当前可宣称目标工作流解耦、胶囊仓库和窄代码页已经实现，不能宣称产品计划或完整 IDE 已实现。后续解耦必须由
...
1. 保持已验收的目标工作流、胶囊仓库和窄代码页；只对尚未实现的产品计划与 IDE 场景制作低保真原型。
...
2. 蛛网仓库与精确入口模块投影已经接入正式只读契约；产品计划助手、灰色缺口生成和行级橙色胶囊仍只能先作为低保真只读原型，原型数据必须清楚标记为 fixture 或占位，不接正式仓库写入。
