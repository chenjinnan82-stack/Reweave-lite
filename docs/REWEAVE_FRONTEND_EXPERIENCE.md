...
- 当前事实基线：2026-07-28，分支 `codex/product-planning-v1`，HEAD `77b348158d6362894e7c89e36e4f50d0ae079372`
- 视觉门状态：第 1 门产品构建流程、第 2 门独立胶囊架与来源证据槽、第 3 门目标接入与全局界面均已获得用户确认；Reweave 本机前端视觉状态为 `PASS`
- 界面用语：用户可见导航与标题统一为“胶囊仓库”；Schema、验收记录和内部变量继续保持既有正式命名
...
- 真实产品目标入口、明确的规划模型选择与探针、必要问题、正式计划审阅、用户确认参数与验收例、候选验证、预览和安全保存。
- `plan_execution.v1/v2` 与 `product_candidate.v1/v2` 的前端状态；候选明确区分 `runtime_operational` 和
  `product_goal_conformance`，全部通过才进入 `review_ready`。
- 真实 SQLite 正式仓库上的只读胶囊仓库场景：第 2 门以“独立胶囊架 + 来源证据槽”为当前视觉权威，包含多来源分组、精确版本正式来源关系、契约展开、搜索、返回和键盘路径。
- 绑定 `capsule_id + version_id + project_id` 的 `capsule_core_code_projection.v1`，只在 active、current、eligible 且 canonical 一致时向现有代码页投影真实 JavaScript 入口模块。
- 已被第 2 门 revision-03 取代的历史方案：黑色蛛网画布、自由缩放／平移画布，以及点击胶囊后以节点镜头直接进入代码页的星丛交互。这些描述仅作历史参考，不得再写成当前决定。
- 统一的冷矿物浅色交付外壳、全局导航、来源入库完整场景、目标四状态和兼容工具降级入口。深色仅承载真实代码与 Diff。
...
`source_workflow.js` 和 `capsule_reader.js` 继续承载各自的窄职责；`capsule_warehouse_scene.js` 已承载只读胶囊仓库、
代码页、正式来源证明、核心代码投影校验和 `getState().warehouse`。

因此，当前可宣称目标工作流解耦、胶囊仓库、窄代码页、正式产品计划场景和候选 Review 已经实现；仍不能宣称完整 IDE、
行级 provenance 或对话式自由修订已经实现。后续解耦必须由
...
### 5.1 当前视觉权威（第 2 门 revision-03，已确认）

当前已确认的胶囊仓库视觉以正式胶囊为第一视觉单位：

1. 来源项目只作为分组外壳（折叠摘要 / 展开货架），不构成仪表盘主体。
2. 展示、交互、计算三类正式胶囊以独立封套排列在胶囊架上；契约可在封缝处展开。
3. 来源槽只证明精确版本、验证代码与失败关闭证据，不承担身份主体。
4. 深色表面只用于真实代码与原始证据，不作为整页仓库背景。

第 1、2、3 门视觉均已获得用户确认。后续产品能力阶段不得以顺手调整共享 CSS 的方式使已确认场景漂移。

### 5.2 已被取代的历史方案

以下内容属于已被第 2 门 revision-03 取代的历史方向，保留仅为审计对照，不得再当作当前实施依据：

- 黑色放射蛛网画布、白色圆形／椭圆节点与星丛镜头动画。
- 自由缩放、平移画布，以及语义缩放驱动的层级浏览。
- 点击胶囊后以节点中心过渡直接进入代码页的空间交互。
- 以蜘蛛织结、装饰连线或空轨卡片填满空白的视觉手法。

正式来源关系、精确版本、资格与验证证据等数据契约不因视觉实现变化而改变。

### 5.3 项目与胶囊层级

胶囊仓库仍有两个语义层级，呈现方式服从已确认实现：

1. 来源分组：默认折叠显示正式来源摘要；展开后显示该来源下的正式胶囊架。
2. 正式胶囊：每枚胶囊独立可读；进入代码页是明确导航，而不是自由画布上的镜头漫游。

项目之间没有经过证明的业务关系时，不绘制项目到项目的数据连线。真实高亮只能表达正式来源、组合或依赖事实。

### 5.4 基础交互

- 不使用持续漂浮的物理模拟，也不允许用户拖动节点并保存另一套图状态。
- 搜索仍然只做事实过滤与定位，不发明第二套关系图。
- 返回必须恢复进入前的来源分组、焦点和键盘位置。
...
当前 v1 只显示正式、当前、精确版本且绑定正式项目来源的 JavaScript `activation.entry_module`。资格、来源、
canonical hash、入口路径或响应 Schema 任一项不能证明时保持“暂无经验证的核心代码”；它不是 HTML/CSS、helper 模块或完整源码浏览器。
...
- 直接定位到该胶囊所属来源项目的胶囊库分组。
- 展开当前来源分组与对应胶囊契约。
- 当前胶囊以琥珀正式来源状态获得焦点。
- 其他胶囊保持中性，不因视觉状态改变正式事实。
...
- 胶囊监督模型位于入库和清洗主线中，只接收经过脱敏的结构化候选摘要，返回严格监督建议。
- 产品规划模型接收产品目标、受控问题和脱敏的胶囊候选摘要，生成计划语义建议；确定性后端独占正式 ID、精确版本绑定、
  依赖、摘要、资格、持久化、失效和确认。

两种角色都不能直接写代码、放宽安全规则、发布胶囊、确认计划、修改验收例或决定验证结果。

### 11.2 未来角色

完整长期愿景还可能增加第三种逻辑角色，但不表示必须运行三个不同的模型或三个服务：
...
相同本地模型运行时可以复用，但每个角色必须拥有独立配置、输入 Schema、输出 Schema、模型 digest、证据和失败策略。
任何既有角色配置都不应被无边界扩大为通用聊天或生成模型。
...
| 场景 | 常驻必要控件 | 按需出现 |
|---|---|---|
| 胶囊仓库 | 搜索、来源分组 | 胶囊契约、重新匹配 |
| 胶囊代码页 | 返回、代码字号、模式切换 | 来源与验证详情 |
...
- 胶囊契约从正式封缝处展开，代码入口保持明确导航。
- IDE 中的橙色胶囊定位到仓库中的同一正式胶囊。
- 返回执行可理解的反向过渡。
- 搜索命中后在普通文档流中定位到目标，不建立虚拟镜头。
...
1. 保持已验收的产品流程、目标工作流、胶囊仓库、代码页和来源入库，不为下一能力顺手重塑已确认界面。
2. 新能力先使用当前后端事实构造只读状态，明确区分正式数据、未来契约和不可实现的占位。
3. 先验证导航、按钮预算、键盘、失效与减少动画体验。
4. 确认出现第二个真实消费者后，再提取最小纯渲染或状态逻辑。
5. 行级 provenance、缺口生成、正式产品晋升和目标副本事务必须分别设计窄契约后接入。

### 18.4 交给主窗口的实施边界

主窗口后续实施以本文作为前端体验北极星，但必须把“当前工作”和“未来原型”分开：

1. 目标工作流解耦阶段只整理已经存在的目标接入工作流、状态失效和渲染职责，不改变计划三后端契约，不增加 bridge 方法，不移动发布 Tag。
2. 胶囊仓库、精确入口模块投影和产品计划助手已经接入正式契约；灰色缺口生成和行级橙色胶囊仍只能先作为低保真只读原型，原型数据必须清楚标记为 fixture 或占位，不接正式仓库写入。
...
2. 只有出现第二个真实消费者时，才从大型控制器中提取对应场景导航和只读展示逻辑。
3. 保住已验收的胶囊仓库、精确来源关系和核心代码投影，不把它扩张为源码浏览器或产品 IDE。
4. 若启动完整 IDE，只先验证文件阅读、橙／灰状态、注释和按钮预算，不伪造行级来源。
5. 根据真实使用暴露的数据缺口，再分别规划窄后端契约。
...
- 返回能恢复来源分组、胶囊契约、文件、行号、滚动和键盘焦点。
- 来源项目节点进入所属胶囊库分组并展开正式胶囊；反向导航恢复进入前的焦点。
- 手风琴章节可通过章节收回控件、返回与安全条件下的 `Esc` 回到同一计划总览状态。
- 搜索、代码字号、注释和模式切换均有真实键盘路径。
- 后端字符串继续以文本方式安全渲染。
- 二进制内容不被前端解码或执行。
...
- 第 1 门产品构建流程视觉已确认，保持冻结。
- 第 2 门“独立胶囊架 + 来源证据槽”和第 3 门“目标接入与全局界面统一”已经确认；黑色蛛网、自由画布和镜头式进代码页已被取代。
- 界面用语统一为“胶囊仓库”。
- 胶囊点击后进入独立代码页（明确导航，而非自由画布镜头漫游）。
- 简单模式与开发者模式保留；目标接入页面可将开发者显示标签表述为“显示技术证据”，不废除同一状态上的双模式契约。
...
- 隔离候选晋升为正式产品与不可变产品版本历史。
- 多 capability Composer 的兼容、依赖和失败关闭契约。
