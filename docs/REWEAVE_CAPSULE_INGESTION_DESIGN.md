...
文档状态：阶段 0 已封板；阶段 1、阶段 2、阶段 3 非活跃基础已完成
...
实施状态：SQLite 单一具体存储、DDL、事务、canonicalization.v1、备份恢复、来源发现、只读快照、extraction_contract.v1、data_contract.v1、敏感决定闭环，以及阶段 3 固定安全清洗、loopback Ollama 监督、Node 计算验证、PySide 图片清洗和真实 QWebEngine 验证均已实现为非活跃基础；阶段 4 前端管理、旧仓导入、阶段 5 生成切换和阶段 6 端到端桌面验收尚未实现
...
本文不是整个产品的实现完成声明。阶段 1 至阶段 3 的非活跃基础已经完成；文中阶段 4 至阶段 6 的目标仍须在后续阶段实现，并用完整桌面流程证明。
...
阶段 0 已完成契约闭合并经用户确认，阶段 1 已完成非活跃 SQLite 基础，阶段 2 已完成非活跃来源和候选基础。SQLite 新路径在安全、模型、验证、资格查询和组合器内存契约完成前不得进入生成活跃路径。
...
原子边界由固定静态规则决定，版本标识为 extraction_contract.v1，正式记录字段为 extraction_contract_version。Ollama 不能选择代码、扩大闭包、删除依赖或决定输入输出契约；它只能在固定提取完成后监督语义、提出名称和分组建议。
...
6. 从固定种子形成候选：render 入口或局部渲染函数是 presentation 种子；根内事件注册和 handler 是 interaction 种子；无 DOM/事件依赖的确定性函数是 computation 种子。
...
- 同一入口模块或其闭包暴露多个 render、mount、compute 角色种子时，以 non_atomic_role_closure_v1 拒绝相关角色；不通过 tree shaking、源码改写或只选择一个 export 伪造原子性。不同角色可以共享只导出纯 helper 的模块。
...
- interaction.input_contract 是 data_contract.v1；output_contract 使用 event_outputs.v1，把每个静态 emit 名称映射到一个 data_contract.v1。
...
event_outputs.v1 的事件名和 error_contract.v1 的错误码必须是静态、非空、唯一字符串；其内部数据节点仍全部遵守 data_contract.v1。presentation 的 error_contract 也使用 error_contract.v1；没有声明错误时 errors 是空对象。
...
- 布局：div、section、article、header、footer
...
1. 每个决定绑定 project_id + source_relpath + source_hash + redaction_rules_version。
2. review_items 保存上述四个绑定值、决定枚举和决定时间；原文仍不保存。
...
7. 新 run 只有四个绑定值完全相同才可复用决定；文件 hash、项目、相对路径或 redaction_rules_version 任一变化，旧决定自动失效并重新 waiting_user。
...
- brand_mode=extend 或 replace 时生成并保存项目自己的 brand_profile_id；projects.brand_profile_json 保存已经物化的完整有效配置，不保存需要运行时合并的增量。
...
模型返回的 capability_kind、名称、分组和 duplicate_suggestions 都只是监督建议：它不能改变 extraction_contract.v1 已确定的代码闭包、DOM 根、依赖、data contract 或合成样例。建议与固定证据冲突时进入 review_required 或 rejected，绝不按模型输出重新切代码边界。
...
8. 使用 backup API 创建并验证恢复前备份。
9. 比较当前数据库和候选恢复库，显示将消失的 capsule、version 和 product usage 数量。
...
| select_supervision_model | 保存 name/digest | 否 |
| list_review_items | 查询脱敏待确认项 | 否 |
| decide_review_item | 提交绑定来源 hash 的 sensitivity/brand 或等价/发布固定决定 | 否 |
...
| start_legacy_import | 创建旧仓导入 run | 是 |
| generate_product | 资格查询、组合、安全写入、usage 记录 | 是 |

桌面使用一个受控任务线程处理扫描、模型和任务编排。它不是后台 watcher。计算、图片和 QWeb 仍通过独立子进程。

decide_review_item 只接收 review_id 和固定决定值；project/path/hash/redaction version 由应用服务从 review_items 读取并绑定，前端不能覆盖这些字段。
...
PySide 真实窗口验收当前只作为本地发布门，不能声称 CI 已覆盖。
...
- 非 loopback URL 拒绝。
- 无法脱敏时 Ollama 未被调用。
- sensitivity/brand/asset 决定只在 project/path/hash/redaction version 全匹配时复用；来源变化后重新 waiting_user；真实记录拒绝优先，其他同绑定冲突失败关闭。
...
阶段 1 至阶段 4 期间，SQLite 目标实现保持非活跃，不通过运行时双仓开关让用户选择。
...
- 内容快照和 no_change。
- extraction_contract.v1、模块闭包和原子候选。
...
- 根内 DOM/Event 白名单封闭。
- extraction_contract.v1 明确固定种子、依赖闭包、契约/样例生成、歧义处理和模型权限，并可触发重扫。
- data_contract.v1 语法和输出到输入的兼容算法封闭。
- waiting_user 的敏感/品牌决定绑定 project/path/hash/redaction version，来源变化后失效。
...
| 敏感确认闭环 | 第 15、18、21、29 节 | 决定绑定 project/path/hash/redaction version；确认后模型仍只看脱敏结构 |
...
- extraction_contract.v1 从明确导出的 `render`、`mount`、`compute` 形成 presentation、interaction、computation 原子候选。同一入口或闭包出现多个角色种子时失败为 non_atomic_role_closure_v1；immutable const 字符串经过有界传播后再进入敏感/品牌证据；入口内未证明属于角色的局部函数失败关闭。interaction 只有返回式 dispose 精确闭合 listener 且 DOM 数值经过 handler 内显式 guard 时才能形成候选。一个角色的边界或契约不能证明时只拒绝该角色，不阻断同一项目其他合法角色。
...
- 敏感门先扫描当前候选涉及的 JS、入口 HTML 和登记 CSS。secret 直接拒绝；真实记录性质、品牌去留或图片像素性质不明确时进入 waiting_user。决定只在 project ID、来源入口、覆盖 HTML/CSS/登记资产/JS 的 source hash 和 redaction_rules.v1 全部相同时复用；真实记录拒绝优先，其他同绑定冲突失败关闭。
...
- 每次处理 review item 都重新只读快照来源、重新执行 extraction_contract.v1 并复算 candidate/source hash。来源快照、候选边界、品牌 profile 或绑定决定变化时不复用旧内存结果，正式写事务前再次复核快照。
