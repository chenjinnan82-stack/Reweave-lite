...
文档状态：阶段 0 已封板；阶段 1–6 在 Static Web V1 支持面内为 `PASS`；本地真实桌面闭环和 GitHub 托管 Ubuntu/Windows CI 已通过；classic script、框架源码与需构建项目明确留在 V1 外
...
Vite 按是否已形成无需构建的原生静态闭包判断，不按名称判断。classic script、React/JSX/TypeScript、需构建框架和多页面推断留在 V1 外；除非用户另行批准新的产品范围和设计契约，否则不进入实现。
